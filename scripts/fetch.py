...
* **The cache is rebuildable and untracked; the manifest is tracked.** `data/raw/`
  ...
  history never shrinks, and the upstream licences (CC BY-SA, and CC BY-NC for
  Memory Alpha) mean this repo must not redistribute the text at all.
...
def stream_to(url: str, dest: Path, *, respect_robots: bool = True) -> Result:
    ...
    The Memory Alpha dump is 82 MB; reading it into a bytes object first is
